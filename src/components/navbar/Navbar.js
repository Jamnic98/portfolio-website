...
function Navbar() {
  ...
  const setDropdownComponent = () => {
    ...
    if (window.innerWidth < 400) {
      ...
      return (
        <div onClick={handleProjectsClick} className={`${navStyles.navLink}`}>
          {projectNavItem}
        </div>
      );
    }
  ...
  return (
    <nav className={`${navStyles.navbar}`}>
      ...
      >
        ...
        <li onMouseLeave={handleMouseLeave} className={`${navStyles.navItem}`}>
          {setDropdownComponent()}
          {dropdown && <Dropdown />}
        </li>
        <li className={`${navStyles.navItem}`}>
          <a
            href='mailto:jamiestimpson30@gmail.com'
            className={`${navStyles.navLink}`}
          >
            <div className={navStyles.icon}>
              <MdEmail />
